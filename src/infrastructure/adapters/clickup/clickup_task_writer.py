...
class ClickUpTaskWriter(ClickUpTaskWriterPort):
    ...
    def __init__(self, http_client: ClickUpHttpClient, members: List[dict] = None, name_to_email: dict = None):
        self._http = http_client
        self.name_to_email = name_to_email
        if name_to_email:
            self.email_to_id = {m["email"].lower(): m["id"] for m in members}
        else:
            self.email_to_id = None

    def set_name_to_email(self, name_to_email: dict) -> None:
        self.name_to_email = name_to_email

    def set_email_to_id(self, members: List[dict]) -> None:
        self.email_to_id = {m["email"].lower(): m["id"] for m in members}

    def apply_results(self, results: List[VerificationResult], folders: List[Folder]) -> None:
    ...
    def _create_task(self, list_id: str, result: VerificationResult) -> None:
        ...
        assignee_ids = resolve_assignee_ids(self.name_to_email, self.email_to_id, assignee_names)
        if assignee_ids:
    ...
    def _update_task(self, result: VerificationResult) -> None:
        ...
        response = self._http.put(endpoint, payload)
